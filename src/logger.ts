import { loadLoggerConfig, type LoggerConfig, type LogDetails, writeToFile } from './config.js';
...
export class Logger {
  ...
  private async ensureConfig(): Promise<void> {
    ...
    if (globalConfig) {
      if (globalConfig.timestamps !== undefined) finalConfig.timestamps = globalConfig.timestamps;
      if (globalConfig.colors !== undefined) finalConfig.colors = globalConfig.colors;
      if (globalConfig.loggingFile !== undefined) finalConfig.loggingFile = globalConfig.loggingFile;
    }

    // Apply user options as overrides (only if explicitly provided)
    if (this.userOptions.level !== undefined) finalConfig.level = this.userOptions.level;
    if (this.userOptions.prefix !== undefined) finalConfig.prefix = this.userOptions.prefix;
    if (this.userOptions.timestamps !== undefined) finalConfig.timestamps = this.userOptions.timestamps;
    if (this.userOptions.colors !== undefined) finalConfig.colors = this.userOptions.colors;
    if (this.userOptions.loggingFile !== undefined) finalConfig.loggingFile = this.userOptions.loggingFile;
  ...
  private formatMessage(level: LogLevel, message: string, includeColors: boolean): string {
    const parts: string[] = [];
  ...
  private async log(level: LogLevel, message: string, ...args: unknown[]): Promise<void> {
    ...
    if (this.loggingFile) {
      const fileOutput = args.length > 0
        ? fileMessage + ' ' + args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' ')
        : fileMessage;
