...
describe('Integration Tests', () => {
  ...
  describe('Full Workflow', () => {
    it('should handle complete logging workflow', async () => {
      ...
      await logger.error('Failed to fetch user data', { error: 'Network error' });
    ...
    it('should handle nested child loggers', async () => {
      const rootLogger = new Logger('/app/server.ts', { prefix: 'Server', level: 'debug' });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle concurrent logging', async () => {
      const logger = new Logger('/test.ts', { loggingFile: path.join(testLogDir, 'concurrent.log') });
  ...
  describe('Config Integration', () => {
    ...
    it('should allow config override via constructor', async () => {
      ...
      const logger2 = new Logger(undefined, { timestamps: false, colors: false });
