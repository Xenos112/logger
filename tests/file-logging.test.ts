...
describe('File Logging Comprehensive Tests', () => {
  ...
  describe('File Path Variations', () => {
    it('should handle absolute paths', async () => {
      ...
      const logger = new Logger(undefined, { loggingFile: logFile, timestamps: false, colors: false });
    ...
    it('should handle relative paths', async () => {
      ...
      const logger = new Logger(undefined, { loggingFile: relativePath, timestamps: false, colors: false });
      
      await logger.info('relative path test');
      await new Promise(r => setTimeout(r, 50));
      
      const expectedPath = path.join(process.cwd(), 'file-test-logs', 'relative.log');
  ...
  describe('File Append Behavior', () => {
    it('should append to existing file', async () => {
      ...
      // First logger
      const logger1 = new Logger(undefined, { loggingFile: logFile, timestamps: false, colors: false });
      await logger1.info('first line');
      await new Promise(r => setTimeout(r, 50));
      
      // Second logger to same file
      const logger2 = new Logger(undefined, { loggingFile: logFile, timestamps: false, colors: false });
